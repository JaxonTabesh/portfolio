import SocialIconComponent, { SocialIcon } from '@/components/social-icon';

export default function Hero() {
  return (
    <section id="0" className="-mt-10 mb-10">
      <div className="flex min-h-screen flex-col items-center justify-center text-center dark:text-neutral-400">
        <h2 className="m-8 text-center text-6xl md:text-9xl">
          <span>{"Hi, I'm "}</span>
          <span className="text-[#e03131]">Jaxon Tabesh</span>.
        </h2>
        <div className="flex items-center gap-6 p-2 text-6xl md:text-8xl">
          <SocialIconComponent type={SocialIcon.GitHub} />
          <SocialIconComponent type={SocialIcon.LinkedIn} />
          <SocialIconComponent type={SocialIcon.Email} />
          <SocialIconComponent type={SocialIcon.Resume} />
        </div>
      </div>
    </section>
  );
}
